...
export function BatchJobSubmission() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!header.includes("receptor") || !header.includes("ligand")) {
          toast.error("CSV must have columns: receptor, ligands, engine (optional)");
          return;
        }

        const parsed: BatchJob[] = [];
        for (let i = 1; i < lines.length; i++) {
          ...
          parsed.push({
            ...
            receptor: cols[0],
            ligands: cols[1],
            engine: cols[2] || "AutoDock Vina",
            status: "pending",
          });
        }

        if (parsed.length === 0) {
          toast.error("No valid job rows found in CSV");
          return;
        }
        if (parsed.length > 50) {
        ...
        setJobs(parsed);
        toast.success(`Loaded ${parsed.length} jobs from CSV`);
      } catch {
    ...
    for (let i = 0; i < jobs.length; i++) {
      ...
      try {
        await orchestrate.mutateAsync({
          workflowType: "docking",
          receptor: job.receptor,
          ligands: job.ligands,
          engine: job.engine,
          binding_site: "Auto-detect pocket",
          priority: "Normal",
          ligandMode: "single",
        });
        setJobs((prev) =>
  ...
  if (!hasBatchAccess) {
    return (
      <div className="rounded-lg border border-border bg-card p-6 text-center">
        ...
        <h3 className="text-sm font-semibold text-foreground mb-1">Batch Docking Submission</h3>
        <p className="text-xs text-muted-foreground mb-3">
          Submit multiple docking jobs at once from a CSV file. Available on Professional and Elite plans.
        </p>
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        CSV format: <code className="bg-muted px-1 rounded text-[10px]">receptor,ligands,engine</code> — one job per row, max 50.
      </p>
      ...
      {jobs.length > 0 && (
        <>
          <div className="max-h-48 overflow-y-auto space-y-1">
            {jobs.map((job, idx) => (
              <div key={job.id} className="flex items-center justify-between rounded bg-muted/50 px-3 py-1.5 text-xs">
                ...
                <span className="flex-1 truncate text-foreground">{job.receptor} → {job.ligands}</span>
                <span className="text-muted-foreground mx-2">{job.engine}</span>
                ...
                {job.status === "pending" && (
                  <DeleteConfirmDialog
                    title="Remove job?"
                    description={`Remove "${job.receptor} → ${job.ligands}" from the batch queue?`}
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground">{jobs.length} jobs · ~{jobs.length * 10} credits</span>
